...
class VdnLanguage 
{
    ...
    const WIDGET_MESSAGES = 'vdnWidgetMessages';
    const PLACEHOLDER = 'placeholder';

    const ERRORS = 'vdnErrorLibrary';
    ...
    /**
     * Static member as set of supported languages for Speech to Text (STT) service by IBM Cloud Watson Platform
     * 
     * Dated: 31/03/2020
     * Reference: https://cloud.ibm.com/docs/speech-to-text?topic=speech-to-text-models
     */
    
    /**
     * Static member method to check given lang code is exist in IBM STT language set
     * 
     * @param String $lang_code  Language code (eg: en-US)
     * 
     * @return Boolean  True if lang code exist otherwise false
     */
    // public static function ibm_supported($lang_code) {
    //     return array_key_exists($lang_code, (array) self::$ibm_language_set);
    // }
     ...
     * 
     * @return Array $vdn_all_lang_set  Array of language arrays
     */
    public static function get_all_languages() {
        $vdn_all_lang_set = array();
        
        try {
    
            if (!empty(self::$gcp_language_set)) {
                $vdn_all_lang_set = self::$gcp_language_set;
            } else {
                $vdn_all_lang_set = array('en-US' => array(self::NAME => 'English (United States)', self::SHORT_PHRASE_LANG => 'en-US', self::LANG_CODE => 'en-US'));
            }
    
            uksort($vdn_all_lang_set, 'strcasecmp');
        } catch(\Exception $err) {
            $vdn_all_lang_set = array('en-US' => array(self::NAME => 'English (United States)', self::SHORT_PHRASE_LANG => 'en-US', self::LANG_CODE => 'en-US'));
        }

        return $vdn_all_lang_set;
    }
    ...
    public static function get_short_phrase_lang_code($lang_code) {
        ...
        $vdn_valid_lang_code = false;

        if (self::gcp_supported($lang_code)) {
            $vdn_lang = self::$gcp_language_set[$lang_code];
            $vdn_valid_lang_code = true;
        }

        if ($vdn_valid_lang_code === true && array_key_exists(self::SHORT_PHRASE_LANG, $vdn_lang)) {
            $short_phrase_lang = $vdn_lang[self::SHORT_PHRASE_LANG];
        }
